  ...
  methods: {
    ...
    getResources (){
      ...
      .then(function (response) {
        self.items = response.data.data.datas
        self.fields = [];
        for(let i=0;i<response.data.data.header.length;i++){
          if(response.data.data.header[i].relation_table !== null){
            self.fields.push({
                key: 'relation_' + response.data.data.header[i].column_name,
                label: response.data.data.header[i].name
            })
          }else{
            self.fields.push({
                key: response.data.data.header[i].column_name,
                label: response.data.data.header[i].name
            })
          }
        }
        ...
        self.maxPages = response.data.data.pagination
