      ...
      <CCard>
        ...
        <CCardBody>
          ...
          <template #status="{item}">
            <td>
              <CBadge :color="getBadge(item.doctorStatus)">
                {{  item.doctorStatus == 1 ? 'unActive':
                    item.doctorStatus == 2 ? 'Active':
                    item.doctorStatus == 3 ? 'Stopped':
                    item.doctorStatus == 4 ? 'Closed':
                    'unActive'}}
                </CBadge>
            </td>
            <td >
            ...
            <td>
              <!-- <CButton class="btn-sm" v-if="you!=item.key" color="danger" @click="deleteDoctor( item.key )">active</CButton> -->
            </td>
          </template>
        </CDataTable>
        </CCardBody>
...
export default {
  ...
  data: () => {
    return {
      items: [],
      fields: ['name', 'email', 'phone', 'department', 'status'],
      currentPage: 1,
      perPage: 10,
      totalRows: 0,
      you: null,
